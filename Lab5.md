...
lab5 <- read.csv("/Users/chenxuejing/Documents/GitHub/Lab5/lab5data.csv")
```
...
``` r
#We should run a simple effect to exmaine the difference between Greek women's GPA and Non-Greek Women's GPA, and examine if the difference is statistically significant.
#The hypothesis is not supported because the p-value is larger than 0.05, which means that the difference between Greek women's GPA and Non-Greek women's GPA is not statistically significant.
...
``` r
#We should run a main effect of gender when looking at the GPA, and Non-Greek Men and Non-Greek Women, .

#Because the main effect doesn't give you the full picture of the results, a simple effect is needed to take a close look at the difference between Greek Women's GPA and Greek Men's GPA, and the difference between Non-Greek Women's GPA and Non-Greek Men's GPA to examine if the two differences are statistically significant.
...
``` r
#The difference between the GPA of Men and Women is statistically insignificant, on average, because the p-value is larger than 0.05.
```
...
ggplot(plot, aes(x = Gender, y = GPA, fill = Gender)) +
  geom_col() + facet_wrap(~ Greek) + theme_bruce()
```
